...
for fold in range(1, 2): # Change 2 back to 105 to training
	data = {}
	#indices = []
	#pdbs = []
	#length_aas = []
	...
	#phis = []
	#pssms = []
	k = 40 #1000 for training, 223 for validation, 30 for testing
	start = 0
	stop = k
	print("Fold: {}".format(fold))
		...
		for chain_index, _ in sorted_pdb_aas:
			if foundmatch == False:
				...
				if pdb_aa and pdb_aa in pnet_aa: # not empty and pdb aa sequence substring of pnet aa sequence
					...
					#dcbeta = get_cbeta_distance_matrix(pdb_path, chain_index)
					dcbeta = None
